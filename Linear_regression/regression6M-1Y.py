...
dfActual_6M = pd.read_csv('/Users/anson/Library/CloudStorage/OneDrive-HKUSTConnect/Yr2 summer/Urop1000/FilesSumUp0206/Data for Regression/Actual-6M.csv')
...
for j in range(len(df.iloc[0]) - 2):
    col_1 = df6M_1Y[df.iloc[0][j + 2]].values.reshape(-1, 1)
    col_2 = dfActual_6M[df.iloc[0][j + 2]].values.reshape(-1, 1)
    x = np.array([])
    y = np.array([])
...
# plt.scatter(slope, t_stat)
# plt.xlabel('Slope')
# plt.ylabel('t-squared')

# # Set the plot title
# plt.title('1M-3M: t-statistic against slope ')
# plt.show()

        
# plt.scatter(slope, R_square)
...
# # Set the plot title
# plt.title('6M-1Y: Slope against R-square')
