...
dfActual_1M = pd.read_csv('/Users/anson/Library/CloudStorage/OneDrive-HKUSTConnect/Yr2 summer/Urop1000/FilesSumUp0206/Data for Regression/Actual-1M.csv')
...
for j in range(len(df.iloc[0]) - 2):
    col_1 = df1M_3M[df.iloc[0][j + 2]].values.reshape(-1, 1)
    col_2 = dfActual_1M[df.iloc[0][j + 2]].values.reshape(-1, 1)
    x = np.array([])
    y = np.array([])
...
# # Set the plot title
# plt.title('1M-3M: Slope against R-square')
